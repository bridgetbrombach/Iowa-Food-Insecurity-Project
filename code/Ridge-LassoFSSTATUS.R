...
cps_data <- cps_data %>% 
  select(c(elderly, married, FSSTATUSMD, black, hispanic, education, 
           female, hhsize, kids, FAMINC))
...
lr_mle <- glm(FSSTATUSMD ~ elderly + married + black + hispanic + education+
                female + hhsize + kids + FAMINC,
...
#Lasso = 0.629
#Ridge = 0.629
#These are lower than the above without FAMINC
