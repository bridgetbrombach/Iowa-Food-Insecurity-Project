...
FSWROUTY_extreme_lasso_rocCurve <- roc(response = as.factor(test.df.preds$FSWROUTY_extreme),
                               predictor = test.df.preds$lasso_pred, 
                               levels = c("0", "1")) 

plot(FSWROUTY_extreme_lasso_rocCurve, main="ROC curve for Lasso model on FSWROUTY_extreme", print.thres = TRUE, print.auc = TRUE)
